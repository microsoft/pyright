...
export namespace UnionType {
    ...
    export function addTypeAliasSource(unionType: UnionType, typeAliasSource: Type) {
        if (typeAliasSource.category === TypeCategory.Union) {
            ...
            if (sourcesToAdd) {
                const updatedEntries = unionType.typeAliasSources ?? [];
                updatedEntries.push(...sourcesToAdd);

                // Dedupe the type alias sources.
                unionType.typeAliasSources = [...new Set(updatedEntries)];
            }
        }
    }
}
...
export function combineTypes(subtypes: Type[], maxSubtypeCount?: number): Type {
    ...
    if (typeAliasSources.length > 0) {
        // Dedupe the type alias sources.
        newUnionType.typeAliasSources = [...new Set(typeAliasSources)];
    }

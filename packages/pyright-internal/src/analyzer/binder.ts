...
interface FinalInfo {
    isFinal: boolean;
    finalTypeNode: ExprNode | undefined;
}

interface ClassVarInfo {
    isClassVar: boolean;
    classVarTypeNode: ExprNode | undefined;
}
...
export class Binder extends ParseTreeWalker {
    ...
    bindModule(node: ModuleNode): void {
        ...
        this._createNewScope(
            ...
            () => {
                ...
                const moduleDocString = ParseTreeUtils.getDocString(node.d.statements);
    ...
    override visitCall(node: CallNode): boolean {
        ...
        if (
            ...
            if (node.d.leftExpr.d.memberName.d.value === 'extend' && node.d.arguments.length === 1) {
                const argExpr = node.d.arguments[0].d.valueExpr;
            ...
            } else if (node.d.leftExpr.d.memberName.d.value === 'remove' && node.d.arguments.length === 1) {
            ...
            } else if (node.d.leftExpr.d.memberName.d.value === 'append' && node.d.arguments.length === 1) {
    ...
    override visitAssignment(node: AssignmentNode): boolean {
        ...
        if (node.d.typeAnnotationComment) {
            ...
            this._addTypeDeclarationForVariable(node.d.leftExpr, node.d.typeAnnotationComment);
        }
        ...
        if (node.d.leftExpr.nodeType === ParseNodeType.Index) {
            ...
            if (
                ...
                if (isCodeFlowSupportedForReference(target.d.baseExpr)) {
                    const baseExprReferenceKey = createKeyForReference(target.d.baseExpr);
        ...
        if (this._currentScope.type === ScopeType.Module) {
            if (
                ...
                if (expr.nodeType === ParseNodeType.List) {
                    ...
                    expr.d.exprs.forEach((tupleEntryNode) => {
                        if (
    ...
    override visitAssignmentExpression(node: AssignmentExprNode) {
        ...
        const evaluationNode = ParseTreeUtils.getEvaluationNodeForAssignmentExpr(node);
        if (!evaluationNode) {
    ...
    override visitTypeAnnotation(node: TypeAnnotationNode): boolean {
        ...
        this._addTypeDeclarationForVariable(node.d.valueExpr, node.d.typeAnnotation);
        ...
        // is unbound.
        const expressionList: CodeFlowReferenceExprNode[] = [];
    ...
    override visitReturn(node: ReturnNode): boolean {
        ...
        if (node.d.returnExpr) {
            this.walk(node.d.returnExpr);
        }
    ...
    override visitIndex(node: IndexNode): boolean {
        ...
        if (this._isTypingAnnotation(node.d.baseExpr, 'Annotated')) {
    ...
    override visitIf(node: IfNode): boolean {
        ...
        postIfLabel.affectedExpressions = this._trackCodeFlowExpressions(() => {
            ...
            const constExprValue = StaticExpressions.evaluateStaticBoolLikeExpr(
                node.d.testExpr,
    ...
    override visitRaise(node: RaiseNode): boolean {
        ...
        if (node.d.tracebackExpr) {
            this.walk(node.d.tracebackExpr);
        }
    ...
    override visitMatch(node: MatchNode) {
        ...
        this.walk(node.d.subjectExpr);

        const expressionList: CodeFlowReferenceExprNode[] = [];
        const isSubjectNarrowable = this._isNarrowingExpression(node.d.subjectExpr, expressionList);
        ...
        node.d.cases.forEach((caseStatement) => {
            ...
            this._createFlowNarrowForPattern(node.d.subjectExpr, caseStatement);
        ...
        if (isSubjectNarrowable) {
            this._createFlowNarrowForPattern(node.d.subjectExpr, node);
        }
    ...
    private _createFlowNarrowForPattern(subjectExpression: ExprNode, statement: CaseNode | MatchNode) {
    ...
    private _createContextManagerLabel(expressions: ExprNode[], isAsync: boolean, blockIfSwallowsExceptions: boolean) {
    ...
    private _bindNeverCondition(node: ExprNode, target: FlowLabel, isPositiveTest: boolean) {
        const expressionList: CodeFlowReferenceExprNode[] = [];
    ...
    private _bindConditional(node: ExprNode, trueTarget: FlowLabel, falseTarget: FlowLabel) {
    ...
    private _createFlowConditional(flags: FlowFlags, antecedent: FlowNode, expression: ExprNode): FlowNode {
        if (antecedent.flags & FlowFlags.Unreachable) {
            return antecedent;
        }
        const staticValue = StaticExpressions.evaluateStaticBoolLikeExpr(
            expression,
            this._fileInfo.executionEnvironment,
            this._fileInfo.definedConstants,
            this._typingImportAliases,
    ...
    private _isLogicalExpression(expression: ExprNode): boolean {
    ...
    private _isNarrowingExpression(
        expression: ExprNode,
        expressionList: CodeFlowReferenceExprNode[],
        ...
        switch (expression.nodeType) {
                ...
                if (isCodeFlowSupportedForReference(expression)) {
                    ...
                    if (!filterForNeverNarrowing) {
                        ...
                        if (
                            ...
                            if (isCodeFlowSupportedForReference(expression.d.baseExpr)) {
                                expressionList.push(expression.d.baseExpr);
                            }
                        }
                    }
                    return true;
            ...
            case ParseNodeType.BinaryOperation: {
                ...
                if (isOrIsNotOperator || equalsOrNotEqualsOperator) {
                    ...
                    if (
                        ...
                        expression.d.leftExpr.d.arguments.length === 1 &&
                        expression.d.leftExpr.d.arguments[0].d.argumentCategory === ArgumentCategory.Simple
                    ) {
                        return this._isNarrowingExpression(
                            expression.d.leftExpr.d.arguments[0].d.valueExpr,
                            expressionList,
                            filterForNeverNarrowing,
                            /* isComplexExpression */ true
                        );
            ...
            case ParseNodeType.Call: {
                if (
                    ...
                    return this._isNarrowingExpression(
                        expression.d.arguments[0].d.valueExpr,
                        expressionList,
                        filterForNeverNarrowing,
                        /* isComplexExpression */ true
                    );
    ...
    private _createAssignmentTargetFlowNodes(target: ExprNode, walkTargets: boolean, unbound: boolean) {
    ...
    private _createFlowAssignment(node: CodeFlowReferenceExprNode, unbound = false) {
    ...
    private _createFlowExhaustedMatch(node: MatchNode) {
        if (!this._isCodeUnreachable()) {
            const flowNode: FlowExhaustedMatch = {
                ...
                node,
                antecedent: this._currentFlowNode!,
                subjectExpression: node.d.subjectExpr,
            };
    ...
    private _bindPossibleTupleNamedTarget(target: ExprNode, addedSymbols?: Map<string, Symbol>) {
        switch (target.nodeType) {
            case ParseNodeType.Name: {
    ...
    private _addInferredTypeAssignmentForVariable(target: ExprNode, source: ParseNode, isPossibleTypeAlias = false) {
        switch (target.nodeType) {
            case ParseNodeType.Name: {
                const name = target;
    ...
    private _addTypeDeclarationForVariable(target: ExprNode, typeAnnotation: ExprNode) {
        ...
        switch (target.nodeType) {
            case ParseNodeType.Name: {
                ...
                if (symbolWithScope && symbolWithScope.symbol) {
                    ...
                    let typeAnnotationNode: ExprNode | undefined = typeAnnotation;
                    ...
                    if (typeAnnotation.nodeType === ParseNodeType.Index) {
                        if (this._isDataclassesAnnotation(typeAnnotation.d.baseExpr, 'InitVar')) {
                            symbolWithScope.symbol.setIsInitVar();
                        } else if (
                            this._isTypingAnnotation(typeAnnotation.d.baseExpr, 'Annotated') &&
                            ...
                            if (
                                ...
                                this._isDataclassesAnnotation(item0Expr.d.baseExpr, 'InitVar')
                            ) {
                                symbolWithScope.symbol.setIsInitVar();
                            }
                        }
    ...
    private _isDataclassesAnnotation(typeAnnotation: ExprNode, name: string): boolean {
    ...
    private _isKnownAnnotation(
        ...
        if (annotationNode.nodeType === ParseNodeType.Name) {
            ...
            if (
                annotationNode.d.leftExpr.nodeType === ParseNodeType.Name &&
    ...
    private _getVariableDocString(node: ExprNode): string | undefined {
    ...
    private _isAnnotationFinal(typeAnnotation: ExprNode | undefined): FinalInfo {
        let isFinal = false;
        let finalTypeNode: ExprNode | undefined;
        ...
        if (typeAnnotation) {
            ...
            if (this._isTypingAnnotation(typeAnnotation, 'Final')) {
                ...
                const finalInfo = this._isAnnotationFinal(typeAnnotation.d.baseExpr);
                if (
                    finalInfo.isFinal &&
    ...
    private _isAnnotationClassVar(typeAnnotation: ExprNode | undefined): ClassVarInfo {
        let isClassVar = false;
        let classVarTypeNode: ExprNode | undefined;
        ...
        while (typeAnnotation) {
            ...
            if (
                ...
                const finalInfo = this._isAnnotationClassVar(typeAnnotation.d.baseExpr);
                if (

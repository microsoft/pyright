...
export class Checker extends ParseTreeWalker {
    ...
    private _isMultipartImportUnused(node: ImportAsNode): boolean {
        ...
        if (!secondLastPartType || !isModule(secondLastPartType)) {
            return false;
        }

        const lastPartName = nameParts[nameParts.length - 1].d.value;
        const symbol =
            secondLastPartType.priv.fields.get(lastPartName) ?? secondLastPartType.priv.loaderFields.get(lastPartName);

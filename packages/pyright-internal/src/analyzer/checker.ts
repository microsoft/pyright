...
export class Checker extends ParseTreeWalker {
    ...
    private _isOverlappingOverload(functionType: FunctionType, prevOverload: FunctionType, partialOverlap: boolean) {
        ...
        if (functionNode) {
            functionType = updateTypeWithInternalTypeVars(
                functionType,
                ParseTreeUtils.getTypeVarScopesForNode(functionNode)
            );
        }

        const prevOverloadNode = prevOverload.shared.declaration?.node?.parent;
        if (prevOverloadNode) {
            prevOverload = updateTypeWithInternalTypeVars(
                prevOverload,
                ParseTreeUtils.getTypeVarScopesForNode(prevOverloadNode)
            );
        }

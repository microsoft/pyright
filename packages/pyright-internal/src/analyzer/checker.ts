...
export class Checker extends ParseTreeWalker {
    ...
    check() {
        // This checker does 2 main works.
        // First is calling type eval on the appropriate nodes so that we can force
        // type evaluation to happen and generate diagnostics if there are issues.
        // Since we are lazy evaluator, unless someone calls type eval, no diagnostic
        // will be generated. This walker basically walks entire tree and force evaluation.
        // Second is using the result of the evaluation, look for issues that require larger
        // context such as whether overloads have consistent signature. or diagnostics that
        // doesn't require internal state of type eval to generate.

        // This walker bypasses visiting ModuleNode, so checking module level complexity
        // directly here.
        if (!this._checkModuleComplexity()) {
            return;
        }
    ...
    override visitClass(node: ClassNode): boolean {
        ...
        // This skips visiting name node to avoid generating unexpected error sometimes on decl name.
        ...
        // Return false because it is doing its own walking (skip visiting name node).
        return false;
    }
    ...
    override visitFunction(node: FunctionNode): boolean {
        ...
        // This does its own walk down of parameters skipping visiting default value.
        node.parameters.forEach((param, index) => {
        ...
        // Return false because it is doing its own walking (skip visiting name node/parameter node and etc).
        return false;
    }
    ...
    override visitLambda(node: LambdaNode): boolean {
        ...
        // It visits all child nodes, but it changes process ordering from pre-order to post-order
        // by doing its own walking.
        return false;
    }
    ...
    override visitName(node: NameNode) {
        // This walker is carefully crafted to avoid calling this method
        // for certain name nodes. (ex, decl name such as class name, import name and etc).
        // For example, calling this over names in ImportFromAs node can cause
        // Undefined variable diagnostic to be generated by type evaluator. Or
        // calling this method without suppressUnboundCheck can cause unexpected
        // diagnostics.
    ...
    override visitImportAs(node: ImportAsNode): boolean {
        ...
        // Don't visit alias name node and visitModuleName will
        // also avoid walking down name node.
        this.walk(node.module);
        return false;
    }
    ...
    override visitImportFrom(node: ImportFromNode): boolean {
        ...
        // Both visitModuleName and visitImportFromAs will avoid
        // walking down any name node.
        this.walk(node.module);
        this.walkMultiple(node.imports);
    ...
    override visitModuleName(node: ModuleNameNode): boolean {
        ...
        // Don't visit each name node in the module name node.
        return false;
    }
    ...
    override visitImportFromAs(node: ImportFromAsNode): boolean {
        ...
        // Don't walk any name under the node.
        return false;
    }
    ...
    private _checkModuleComplexity() {

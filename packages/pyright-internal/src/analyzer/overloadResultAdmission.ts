 ...
 *
 * Conservative local-use admission for overload alternatives.
 */
...
export interface LocalOverloadAdmission {
    ...
    dependencies: Map<LocalOverloadOperation, ReadonlySet<LocalOverloadOperation>>;
    graphUnits: number;
    complete: boolean;
    reason?: string;
}
...
export function admitLocalOverloadResults(
    ...
    try {
        ...
        for (const record of result.records.values()) {
            ...
            result.dependencies.set(record, dependencies);
            record.admitted =

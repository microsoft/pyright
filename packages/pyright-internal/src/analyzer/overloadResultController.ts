 ...
 *
 * Bounded local overload alternatives and operation control.
 */
...
import {
    ...
    findAutomaticOverloadSeeds,
    hasNestedOverloadAny,
    isAutomaticOverloadSeedSyntax,
...
export interface ExperimentalOverloadResultOptions {
    ...
    // Separate test-only experiment; never implied by automatic activation.
    checkerHandoff?: boolean;
    observe?: (event: OverloadResultEvent) => void;
}

export type OverloadResultEvent =
    | { kind: 'activationProbe'; node: CallNode; selection: AutomaticOverloadSelection }
...
interface Generation {
    ...
    ordinaryQueryCaches: Map<NameNode, () => boolean>;
    work: OverloadResultWork;
}
interface ActiveOperation {
    generation: Generation;
    record: LocalOverloadOperation;
    parent?: ActiveOperation;
    previousSink: TextRangeDiagnosticSink;
    mode: OperationMode;
...
export class ExperimentalOverloadResultController implements EvaluationOperationController {
    ...
    private _activationBoundary = false;
    private _activationCanRestart = false;
    private _canActivate?: () => boolean;
    private _flushActivation?: () => void;
    ...
    private _automaticWork = {
        ...
        selected: 0,
        proofUnits: 0,
        activationArgumentChecks: 0,
        activationProbes: 0,
        activationProofUnits: 0,
        activations: 0,
    ...
    get isAutomatic() {
        return !!this._options.automatic;
    }

    get hasPendingActivation() {
    ...
    installActivationBoundary = (canActivate: () => boolean, flush: () => void) => {
        this._canActivate = canActivate;
        this._flushActivation = flush;
    };

    installQueryCache = (capture: (root: ParseNode, node: NameNode) => (() => boolean) | undefined) => {
        this._captureQueryCache = capture;
    };

    installCheckerHandoff = (
        handoff: (root: ParseNode, expression: ExpressionNode, result: TypeResult, callback: () => void) => boolean
    ) => {
        this._checkerHandoff = handoff;
    };
    ...
    getDiagnosticSink(node: ParseNode) {
        const active = this._active;
        const record = active?.generation.admission.byNode.get(node);
        if (
            !active ||
            !record ||
            this._isEnabled(active.generation, record) ||
            record.fn !== active.record.fn ||
            record.root.start >= active.record.root.start
        ) {
            return undefined;
        }
        // Earlier ordinary flow prerequisites keep their native caches. Their
        // diagnostics must likewise outlive this operation's private trial.
        let outer = active;
        while (outer.parent?.generation === active.generation) {
            outer = outer.parent;
        }
        return outer.previousSink;
    }

    needsActivationBoundary() {
        return this._options.automatic && !this._activationBoundary && !this._active;
    }

    runWithActivation<T>(callback: () => T): T {
        assert(!this._disposed, 'Retired overload-result controller');
        assert(!this._activationBoundary);
        this._activationBoundary = true;
        try {
            this._activationCanRestart = this._canActivate!();
            while (true) {
                try {
                    if (this._activationCanRestart && this.hasPendingActivation) {
                        this._flushActivation!();
                    }
                    return callback();
                } catch (error) {
                    if (!(error instanceof ActivationRestart)) {
                        throw error;
                    }
                }
            }
        } finally {
            this._activationBoundary = false;
            this._activationCanRestart = false;
        }
    }

    considerActivation(node: CallNode, args: readonly { argType: Type }[]) {
        if (
            !this._options.automatic ||
            (this._active &&
                (!this._activationCanRestart ||
                    this._active.parent ||
                    this._active.mode !== 'baseline' ||
                    this._active.record.ids.has(node.id))) ||
            this._discovering ||
            !this._activationBoundary ||
            this._activationChecked.has(node) ||
            !isAutomaticOverloadSeedSyntax(node)
        ) {
            return;
        }
        this._automaticWork.activationArgumentChecks++;
        if (
            !args.some(
                (arg) =>
                    isClassInstance(arg.argType) &&
                    arg.argType.priv.typeArgs?.length &&
                    isFixedOverloadArgumentShape(arg.argType, new OverloadSelectionBudget(this._limits.proofUnits)) &&
                    hasNestedOverloadAny(arg.argType)
            )
        ) {
            return;
        }
        ...
        try {
            if (!findAutomaticOverloadSeeds([node], this._evaluator()).length) {
                return;
            }
            const selection = this._select!(node, this._limits.candidates, this._limits.proofUnits);
            this._automaticWork.activationProbes++;
            this._automaticWork.activationProofUnits += selection.units;
            this._emit({ kind: 'activationProbe', node, selection });
            if (selection.reason === 'selected') {
                this._activationChecked.add(node);
                this._pendingActivation.add(node);
                // Unwind native speculation before it can publish or cache
                // further uses of the provisional ordinary result.
                if (this._activationCanRestart) {
                    throw new ActivationRestart();
                }
            }
        } finally {
    ...
    finishActivation() {
        if (this._active || this._discovering || !this._pendingActivation.size) {
            return;
        }
        let activated = false;
        this._discovering = true;
        try {
            for (const node of this._pendingActivation) {
                this._activated.add(node);
                const generation = this._ensure(node, true);
                const record = generation?.admission.byNode.get(node);
                if (generation && record?.admitted && record.seed?.node === node) {
                    this._reserveGeneration(generation);
                    this._automaticWork.activations++;
                    activated = true;
                } else {
                    this._activated.delete(node);
                }
            }
            this._pendingActivation.clear();
        } finally {
            this._discovering = false;
        }
        if (activated) {
    ...
    route = (request: EvaluationOperationRequest): { result?: TypeResult } | undefined => {
        ...
        if (
            !generation ||
            !record ||
            !this._isEnabled(generation, record) ||
            record.assignment !== request.node ||
            this._active?.record === record
        ) {
            return undefined;
        }
    ...
    check = (module: ModuleNode) => {
        const initialGeneration = this._ensure(module);
        if (!initialGeneration && !this._options.automatic) {
            return undefined;
        }
        initialGeneration?.published.clear();
        const walk = (node: ParseNode, callback: () => void): void => {
            if (node.nodeType === ParseNodeType.StatementList && this.needsActivationBoundary()) {
                return this.runWithActivation(() => walk(node, callback));
            }
            const generation = this._ensure(module);
            const record = generation?.admission.records.get(node);
            if (this._active || !generation || !record || !this._isEnabled(generation, record)) {
                callback();
                return;
            }
            let handedOff: OrdinaryOutcome | undefined;
            const canHandoff =
                this._options.automatic &&
                this._options.checkerHandoff === true &&
                !this._options.observe &&
                !record.assignment &&
                !record.seed &&
                !record.reader &&
                !record.context &&
                !record.flags &&
                record.root.nodeType === ParseNodeType.StatementList &&
                record.statement === record.expression &&
                record.expression.nodeType === ParseNodeType.Call;
            const output = this._build(
                generation,
                record,
                canHandoff
                    ? (inputs, candidate, result, diagnostics) => {
                          if (diagnostics.length || result.typeErrors || result.isIncomplete) {
                              return;
                          }
                          this._checkerHandoff?.(record.root, record.expression, result, () => {
                              handedOff = this._trial(
                                  generation,
                                  record,
                                  'check-candidate',
                                  inputs,
                              ...
                              generation.work.checkerHandoffs++;
                          });
                      }
                    : undefined
            );
            ...
            const committed =
                handedOff ?? this._trial(generation, record, mode, output.inputs, selected?.candidate, callback);
    ...
    private _retire(generation: Generation) {
        ...
        generation.ordinaryQueryCaches.clear();
    }

    private _ensure(node: ParseNode, activate = false): Generation | undefined {
        if (this._options.automatic && !activate && !this._generations.size) {
            return undefined;
        }
        let root = node;
        while (root.parent) {
        ...
        if (this._options.automatic && !activate) {
            return undefined;
        }
    ...
    private _isEnabled(generation: Generation, record: LocalOverloadOperation) {
        return (
            record.admitted &&
            (!this._options.automatic ||
                [...(generation.admission.dependencies.get(record) ?? [])].every(
                    (origin) => !!origin.seed && this._activated.has(origin.seed.node)
                ))
        );
    }

    private _reserveGeneration(generation: Generation) {
        ...
        for (const record of generation.admission.records.values()) {
            if (!this._isEnabled(generation, record)) {
                continue;
            }
    ...
    private _inputs(generation: Generation, record: LocalOverloadOperation): Inputs {
        ...
        for (const node of record.nodes) {
            ...
            if (!producer || !this._isEnabled(generation, producer) || producer === record) {
                continue;
            }
    ...
    private _trial(
        ...
        retain = false,
        reservation?: Reservation,
        continuation?: (result: TypeResult, diagnostics: readonly Diagnostic[]) => void,
        liveCheckerCache = false
    ): OrdinaryOutcome {
        ...
        assert(
            !liveCheckerCache ||
                (parent?.record === record &&
                    parent.mode === 'candidate' &&
                    mode === 'check-candidate' &&
                    !nestedSpeculative &&
                    !retain &&
                    !reservation)
        );
        ...
        try {
            ...
            const evaluateAndContinue = () => {
                const value = evaluate();
                if (continuation) {
                    diagnostics = sink.fetchAndClear();
                    continuation(value, diagnostics);
                }
                return value;
            };
            result =
        ...
        if (!this._active && !generation.building.size && !generation.preparing) {
            ...
            try {
                for (const prior of generation.admission.records.values()) {
                    if (
                        this._isEnabled(generation, prior) &&
                        prior.fn === record.fn &&
                        prior.root.start < record.root.start
                    ) {
                ...
                try {
                    ...
                    for (const [index, candidate] of candidates.entries()) {
                        ...
                        const outcome = this._trial(
                            ...
                            false,
                            reservation,
                            checkerContinuation && !successes.length && index === candidates.length - 1
                                ? (result, diagnostics) => checkerContinuation(inputs, candidate, result, diagnostics)
                                : undefined
                        );
    ...
    private _queryOutcome(generation: Generation, record: LocalOverloadOperation, queryNode?: NameNode): TypeResult {
        const outcome = this._build(generation, record);
        if (!isOverloadResult(outcome.canonical.type) && !outcome.inputs.alternative) {
            if (queryNode && generation.ordinaryQueryCaches.get(queryNode)?.()) {
            ...
            if (queryNode) {
                generation.ordinaryQueryCaches.delete(queryNode);
                const guard = !result.isIncomplete && this._captureQueryCache?.(record.root, queryNode);
                if (guard) {
                    generation.ordinaryQueryCaches.set(queryNode, guard);
                }
            }
            return result;
        }
    ...
    private _query(
        ...
        const record = generation?.admission.byNode.get(node);
        if (!generation || !record || !this._isEnabled(generation, record) || !record.valueNodes.has(node)) {
            return undefined;
        }
        ...
        if (node.nodeType === ParseNodeType.Name) {
            ...
            const value =
                definition && this._isEnabled(generation, definition)
                    ? this._build(generation, definition).value
                    : undefined;
            if (value && !isOverloadResult(value.type)) {
                // Only public context-free Name readers can reuse a previously
                // owned restoration. The ordinary reader still returns the
                // actual cache entry; expression/flag requests always evaluate.
                this._queryOutcome(generation, record, publicQuery ? node : undefined);
            }

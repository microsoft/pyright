...
import { ArgumentCategory, ExprNode, NameNode, NumberNode, ParseNodeType, TupleNode } from '../parser/parseNodes';
import { KeywordType, OperatorType } from '../parser/tokenizerTypes';

// Returns undefined if the expression cannot be evaluated
// statically as a bool value or true/false if it can.
export function evaluateStaticBoolExpr(
    node: ExprNode,
    ...
    if (node.nodeType === ParseNodeType.AssignmentExpr) {
        return evaluateStaticBoolExpr(
            node.d.rightExpr,
            execEnv,
            definedConstants,
            typingImportAliases,
    ...
    if (node.nodeType === ParseNodeType.UnaryOperation) {
        if (node.d.operator === OperatorType.Not) {
            const value = evaluateStaticBoolLikeExpr(
                node.d.expr,
                execEnv,
                definedConstants,
                typingImportAliases,
        ...
        if (node.d.operator === OperatorType.Or || node.d.operator === OperatorType.And) {
            const leftValue = evaluateStaticBoolExpr(
                node.d.leftExpr,
                execEnv,
                definedConstants,
                typingImportAliases,
            ...
            const rightValue = evaluateStaticBoolExpr(
                node.d.rightExpr,
                execEnv,
                definedConstants,
                typingImportAliases,
        ...
        if (
            _isSysVersionInfoExpr(node.d.leftExpr, sysImportAliases) &&
        ...
        if (
            ...
            _isSysVersionInfoExpr(node.d.leftExpr.d.baseExpr, sysImportAliases) &&
            node.d.leftExpr.d.items.length === 1 &&
            !node.d.leftExpr.d.trailingComma &&
            !node.d.leftExpr.d.items[0].d.name &&
            node.d.leftExpr.d.items[0].d.argumentCategory === ArgumentCategory.Simple &&
        ...
        if (
            _isSysPlatformInfoExpr(node.d.leftExpr, sysImportAliases) &&
        ...
        if (_isOsNameInfoExpr(node.d.leftExpr) && node.d.rightExpr.nodeType === ParseNodeType.StringList) {
            ...
            if (node.d.rightExpr.nodeType === ParseNodeType.StringList) {
                ...
                if (node.d.leftExpr.nodeType === ParseNodeType.Name) {
                    ...
                    constantValue = definedConstants.get(node.d.leftExpr.d.memberName.d.value);
                }
...
// Similar to evaluateStaticBoolExpr except that it handles
// other non-bool values that are statically falsy or truthy
// (like "None").
export function evaluateStaticBoolLikeExpr(
    node: ExprNode,
    execEnv: ExecutionEnvironment,
    definedConstants: Map<string, boolean | string>,
    typingImportAliases?: string[],
    ...
    return evaluateStaticBoolExpr(node, execEnv, definedConstants, typingImportAliases, sysImportAliases);
}

function _convertTupleToVersion(node: TupleNode): PythonVersion | undefined {
    if (node.d.exprs.length >= 2) {
        if (
            node.d.exprs[0].nodeType === ParseNodeType.Number &&
            !node.d.exprs[0].d.isImaginary &&
            node.d.exprs[1].nodeType === ParseNodeType.Number &&
            !node.d.exprs[1].d.isImaginary
        ) {
            const majorNode = node.d.exprs[0];
            const minorNode = node.d.exprs[1];
            ...
            if (
                node.d.exprs.length >= 3 &&
                node.d.exprs[2].nodeType === ParseNodeType.Number &&
                !node.d.exprs[2].d.isImaginary &&
                typeof node.d.exprs[2].d.value === 'number'
            ) {
                micro = node.d.exprs[2].d.value;
            }

            let releaseLevel: PythonReleaseLevel | undefined;
            if (
                node.d.exprs.length >= 4 &&
                node.d.exprs[3].nodeType === ParseNodeType.StringList &&
                node.d.exprs[3].d.strings.length === 1 &&
                node.d.exprs[3].d.strings[0].nodeType === ParseNodeType.String
            ) {
                releaseLevel = node.d.exprs[3].d.strings[0].d.value as PythonReleaseLevel;
            }

            let serial: number | undefined;
            if (
                node.d.exprs.length >= 5 &&
                node.d.exprs[4].nodeType === ParseNodeType.Number &&
                !node.d.exprs[4].d.isImaginary &&
                typeof node.d.exprs[4].d.value === 'number'
            ) {
                serial = node.d.exprs[4].d.value;
            }

            return new PythonVersion(major, minor, micro, releaseLevel, serial);
        }
    } else if (node.d.exprs.length === 1) {
        const major = node.d.exprs[0] as NumberNode;
...
function _isSysVersionInfoExpr(node: ExprNode, sysImportAliases: string[] = ['sys']): boolean {
    if (node.nodeType === ParseNodeType.MemberAccess) {
        if (node.d.leftExpr.nodeType === ParseNodeType.Name && node.d.memberName.d.value === 'version_info') {
...
function _isSysPlatformInfoExpr(node: ExprNode, sysImportAliases: string[] = ['sys']): boolean {
    if (node.nodeType === ParseNodeType.MemberAccess) {
        if (node.d.leftExpr.nodeType === ParseNodeType.Name && node.d.memberName.d.value === 'platform') {
            if (sysImportAliases.some((alias) => alias === (node.d.leftExpr as NameNode).d.value)) {
                return true;
            }
        }
...
function _isOsNameInfoExpr(node: ExprNode): boolean {
    if (node.nodeType === ParseNodeType.MemberAccess) {
        if (
            node.d.leftExpr.nodeType === ParseNodeType.Name &&

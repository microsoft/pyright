...
[false, true].forEach((minimizeResults) => {
    describe(`Tokenizer with ${minimizeResults}==minimize`, () => {
        ...
        test('Strings: unclosed', () => {
            const results = tokenize(' "string" """line1\n#line2"""\t\'un#closed');
        ...
        test('Strings: block next to regular, double-quoted', () => {
            const results = tokenize('"string""""s2"""');
        ...
        test('Strings: single quoted f-string', () => {
            const results = tokenize("a+f'quoted'");
            assert.equal(results.tokens.count, 5 + _implicitTokenCount);
            assert.equal(results.tokens.getItemAt(0).type, TokenType.Identifier);
        ...
        test('Strings: double quoted f-string', () => {
            const results = tokenize('x(1,f"quoted")');
            assert.equal(results.tokens.count, 8 + _implicitTokenCount);
            assert.equal(results.tokens.getItemAt(0).type, TokenType.Identifier);
            assert.equal(results.tokens.getItemAt(1).type, TokenType.OpenParenthesis);
        ...
        test('Strings: single quoted multiline f-string', () => {
            const results = tokenize("f'''quoted'''");
            assert.equal(results.tokens.count, 3 + _implicitTokenCount);
        ...
        test('Strings: f-string with single right brace', () => {
            const results = tokenize("f'hello}'");
            assert.equal(results.tokens.count, 4 + _implicitTokenCount);
        ...
        test('Strings: f-string with escape in format string 1', () => {
            const results = tokenize("f'he\\{ 1 }lo'");
        ...
        test('Strings: f-string with double brace', () => {
            const results = tokenize(`f"hello {{{0==0}}}"`);
        ...
        test('Strings: f-string with unterminated expression', () => {
            const results = tokenize("f'hello { a'");
            assert.equal(results.tokens.count, 5 + _implicitTokenCount);
        ...
        test('Strings: f-string with replacement field', () => {
            const results = tokenize("f'hello { a + b}'");
            assert.equal(results.tokens.count, 8 + _implicitTokenCount);
        ...
        test('Strings: nested f-string formats 1', () => {
            const results = tokenize("f'{a:x{{b}+:x{c}+}}'");
        ...
        test('Strings: nested f-string formats 2', () => {
            const results = tokenize("f'hi{'x':*^{8:{'':}}0}'");
        ...
        test('Strings: b/u/r-string', () => {
            const results = tokenize('b"b" U\'u\' bR"br" Ru\'ur\'');
            assert.equal(results.tokens.count, 4 + _implicitTokenCount);
            ...
            assert.equal(
                stringToken2.flags,
                StringTokenFlags.DoubleQuote | StringTokenFlags.Bytes | StringTokenFlags.Raw
            );
            ...
            assert.equal(
                stringToken3.flags,
                StringTokenFlags.SingleQuote | StringTokenFlags.Unicode | StringTokenFlags.Raw
            );
        ...
        test('Strings: bytes string with non-ASCII', () => {
            const results = tokenize("B\"Teßt\" b'''Teñt'''");
            assert.equal(results.tokens.count, 2 + _implicitTokenCount);
        ...
        test('Strings: invalid escape characters', () => {
            ...
            assert.equal(
                unescapedValue.unescapeErrors[1].errorType,
                StringTokenUtils.UnescapeErrorType.InvalidEscapeSequence
            );
        });

        test('Strings: good hex escapes', () => {
            const results = tokenize('"\\x4d" "\\u006b" "\\U0000006F"');
            assert.equal(results.tokens.count, 3 + _implicitTokenCount);
        ...
        test('Strings: bad hex escapes', () => {
            const results = tokenize('"\\x4g" "\\u006" "\\U0000006m"');
            assert.equal(results.tokens.count, 3 + _implicitTokenCount);
        ...
        test('Strings: good name escapes', () => {
            const results = tokenize('"\\N{caret escape blah}" "a\\N{A9}a"');
            assert.equal(results.tokens.count, 2 + _implicitTokenCount);
        ...
        test('Strings: bad name escapes', () => {
            const results = tokenize('"\\N{caret" "\\N{.A9}"');
            assert.equal(results.tokens.count, 2 + _implicitTokenCount);
        ...
        test('Comments', () => {
            const results = tokenize(' #co"""mment1\n\t\n#x\'y2 ');
            assert.equal(results.tokens.count, 1 + _implicitTokenCountNoImplicitNewLine);
            assert.equal(results.tokens.getItemAt(0).type, TokenType.NewLine);
        });

        test('Period to operator token', () => {
            const results = tokenize('x.y');
            assert.equal(results.tokens.count, 3 + _implicitTokenCount);
        ...
        test('Hex number', () => {
            const results = tokenize('1 0X2 0xFe_Ab 0x');
            assert.equal(results.tokens.count, 5 + _implicitTokenCount);
        ...
        test('Binary number', () => {
            const results = tokenize('1 0B1 0b010 0b3 0b');
            assert.equal(results.tokens.count, 7 + _implicitTokenCount);
        ...
        test('Octal number', () => {
            const results = tokenize('1 0o4 0O0_7_7 -0o200 0o9 0oO');
            assert.equal(results.tokens.count, 9 + _implicitTokenCount);
        ...
        test('Decimal number', () => {
            const results = tokenize('-2147483647 ++2147483647');
            assert.equal(results.tokens.count, 5 + _implicitTokenCount);
        ...
        test('Floating point number', () => {
            const results = tokenize('3.0 .2 ++.3e+12 --.4e1 1e-4 0.01 01.0');
            assert.equal(results.tokens.count, 11 + _implicitTokenCount);
        ...
        test('Floating point numbers with parens', () => {
            const results = tokenize('(3.0) (.2) (+.3e+12, .4e1; 0)');
        ...
        test('Floating point numbers with operators', () => {
            const results = tokenize('88.9/100.0*4.0-2.0,');
            assert.equal(results.tokens.count, 8 + _implicitTokenCount);
        ...
        test('Imaginary numbers', () => {
            const results = tokenize('88.9j/100.0J*4.0e-5j-2.0j,');
            assert.equal(results.tokens.count, 8 + _implicitTokenCount);
        ...
        test('Underscore numbers', () => {
            const results = tokenize('1_0_0_0 0_0 .5_00_3e-4 0xC__A_FE_F00D 10_000_000.0 0b_0011_1111_0100_1110');
        ...
        test('Operators', () => {
            const text =
                ...
                '& &= | |= ^ ^= ' +
                ':= <>';
            const results = tokenize(text);
            const lengths = [
                1, 2, 3, 2, 2, 1, 2, 3, 2, 2, 1, 1, 1, 1, 1, 2, 1, 2, 2, 3, 2, 2, 2, 2, 3, 1, 2, 1, 2, 1, 2, 2, 2,
            ];
            const operatorTypes = [
        ...
        test('Identifiers', () => {
            const results = tokenize('and __and __and__ and__');
            assert.equal(results.tokens.count, 4 + _implicitTokenCount);
        ...
        test('Lines1', () => {
            ...
            const resultsLf = tokenize(sampleTextLfOnly);
            ...
            const resultsCrLf = tokenize(sampleTextCrLf);
        ...
        test('Comments2', () => {
            const results = tokenize('class A:\n    def func(self):\n        pass\n        # comment\n    ');
            assert.equal(results.tokens.count, 16 + _implicitTokenCount);
        ...
        test('Identifiers1', () => {
            const results = tokenize('배열 数値 лік Opciók 可選值');
            assert.equal(results.tokens.count, 5 + _implicitTokenCount);
        ...
        test('TypeIgnoreLine2', () => {
            const results = tokenize('a = 3 # type: ignores\n"test" # type:ignore');
        ...
        test('Constructor', () => {
            const results = tokenize('def constructor');
            assert.equal(results.tokens.count, 2 + _implicitTokenCount);

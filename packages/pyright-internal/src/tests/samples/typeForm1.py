...
s1 = TypeForm(int)
reveal_type(s1, expected_text="type[int]")

s2 = TypeForm("int | str")
reveal_type(s2, expected_text="Literal['int | str'] & TypeForm[int | str]")

s3 = TypeForm(list["str"])
reveal_type(s3, expected_text="type[list[str]]")

s4 = TypeForm(Annotated[int, "meta"])
reveal_type(s4, expected_text="Annotated & TypeForm[int]")

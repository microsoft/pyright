...
import { CaseSensitivityDetector, FileSystem, MkDirOptions, TempFile, TmpfileOptions } from './fileSystem';
import {
    FileWatcher,
...
class RealFileSystem implements FileSystem {
    ...
    realpathSync(uri: Uri) {
        try {
            const path = uri.getFilePath();
            return Uri.file(yarnFS.realpathSync(path), uri.isCaseSensitive);
        } catch (e: any) {
            return uri;
        }
    }
    ...
    getModulePath(): Uri {
        ...
        return getRootUri(this._caseSensitiveDectector.isLocalFileSystemCaseSensitive()) || Uri.empty();
    }
    ...
    realCasePath(uri: Uri): Uri {
        try {
            ...
            return Uri.file(realCase, uri.isCaseSensitive);
        } catch (e: any) {
...
export class RealTempFile implements TempFile, CaseSensitivityDetector {
    ...
    tmpdir(): Uri {
        return Uri.file(this._getTmpDir().name, this.isLocalFileSystemCaseSensitive());
    }

    tmpfile(options?: TmpfileOptions): Uri {
        const f = tmp.fileSync({ dir: this._getTmpDir().name, discardDescriptor: true, ...options });
        return Uri.file(f.name, this.isLocalFileSystemCaseSensitive());
    }
    ...
    isCaseSensitive(uri: string): boolean {
        if (uri.startsWith(FileUriSchema)) {
            return this.isLocalFileSystemCaseSensitive();
        }

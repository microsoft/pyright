...
4. If more than one overload remains, the “winner” is chosen based on the order in which the overloads are declared. In general, the first remaining overload is the “winner”. There are three exceptions to this rule.
    ...
    Exception 2: When multiple overloads match because an argument contains `Any` or `Unknown` in an invariant type argument, pyright applies the [overload materialization rule](https://typing.python.org/en/latest/spec/overload.html#step-5). An overload that accepts all possible materializations of every argument eliminates all subsequent overloads. If multiple overloads with non-equivalent return types remain, pyright preserves shared generic return structure where possible, replacing incompatible invariant type arguments with `Any` or `Unknown`. Compatible tuple shapes can also be preserved. For example, return types `list[int]` and `list[str]` produce `list[Any]` when the ambiguity comes from a nested `Any`, retaining attribute checking on the list. If no supported common structure exists, the call evaluates to `Any` when the ambiguity is caused by a nested `Any`, or `Unknown` when it is caused by a nested `Unknown`. This also applies to specialized `self` parameters and inferred constructor results.

...
**autoExcludeVenv** [boolean, optional]: Specifies whether directories detected as virtual environments should be excluded. This defaults to false when directories are listed in the **exclude** setting, otherwise it defaults to true. When enabled, any directory that contains a `pyvenv.cfg`, `bin/activate` or `scripts/activate` file will be automatically excluded.

...
/** @type {(packageFile: string, transitive: boolean, reject?: string[]) => Promise<boolean>} */
async function updatePackage(packageFile, transitive, reject = undefined) {
    packageFile = path.resolve(packageFile);
    const packageName = path.basename(path.dirname(packageFile));
    ...
    if (!transitive && Object.keys(/**@type {any}*/ (updateResult)).length === 0) {
        // If nothing changed and we aren't updating transitive deps, don't run pnpm install.
        return false;
    }
...
async function updateAll(transitive, reject = undefined) {
    ...
    const updatedPackages = await Promise.all(
        packageFiles.map((packageFile) => updatePackage(packageFile, transitive, reject))
    );

    if (transitive) {
        console.log('removing pnpm-lock.yaml and node_modules');
        await fsAsync.unlink('pnpm-lock.yaml');
        await rmdir('node_modules');
    }

    if (transitive || updatedPackages.some((updated) => updated)) {

...
| Import resolver caches | Import resolver state is outside `SourceFile`/`SourceFileInfo`, and `emptyCache()` now clears resolver-owned import-result/module-name/parent-directory caches. | Add real-project stats to quantify resolver cache size and decide whether cache usage should contribute to the low-memory high-water mark. |
| Edit-mode snapshots | Edit mode intentionally keeps pre-edit state alive until exit. | Confirm long edit-mode sessions do not accumulate multiple generations per file. |

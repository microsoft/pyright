...
| Import resolver caches | Import resolver state is outside `SourceFile`/`SourceFileInfo`. | Audit whether resolver caches duplicate large `ImportResult` graphs or are mostly path/package metadata. |
